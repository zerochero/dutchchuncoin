...
#if defined(NDEBUG)
# error "Dutchchuncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x568b3a88dc0fedb344546e44aa41d1ebafb34bc5be2051388922e331d5c09fd6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dutchchuncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dutchchuncoin Signed Message:\n";
...
{
    // Dutchchuncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 28 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 10000); // Dutchchuncoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Dutchchuncoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Dutchchuncoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dutchchuncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dutchchuncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xabf01adc5e1607a227dfeb0d701a9065fb0f53300352e3d0f09ae73aee74ba6a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1517814296;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084899638;

        if (fTestNet)
        {
            block.nTime    = 1517814279;
            block.nNonce   = 385767150;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x51d97ae1434bb971d7c468d5d1ff15de402ae7139d9b0f630892e0b0bc5127ba"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Dutchchuncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DutchchuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DutchchuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DutchchuncoinMiner(CWallet *pwallet)
{
    printf("DutchchuncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dutchchuncoin-miner");
    ...
    try { loop {
        ...
        printf("Running DutchchuncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DutchchuncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DutchchuncoinMiner, pwallet));
}
